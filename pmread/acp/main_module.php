...
class main_module
{
	...
	*/
	protected function print_view(array $rows, $truncated_at = 0)
	{
		...
		if (!count($rows))
		{
			echo '<p>' . htmlspecialchars($this->user->lang('NO_MESSAGES'), ENT_QUOTES, 'UTF-8') . '</p>';
		}
		else
		{
			echo '<table class="grid">' . "\n";
			echo '<thead><tr>'
				. '<th class="c-id">' . $this->esc($this->user->lang('MSG_ID')) . '</th>'
				. '<th class="c-date">' . $this->esc($this->user->lang('DATETIME')) . '</th>'
				. '<th>' . $this->esc($this->user->lang('FROM')) . '</th>'
				. '<th>' . $this->esc($this->user->lang('TO')) . '</th>'
				. '<th>' . $this->esc($this->user->lang('PMREAD_EXPORT_COL_BCC')) . '</th>'
				. '<th>' . $this->esc($this->user->lang('SUBJECT')) . '</th>'
				. '</tr></thead>' . "\n<tbody>\n";
...
';
	...
	*/
	protected function export_csv(array $msg_ids, $sql_where = '')
	{
		...
		fputcsv($out, array(
			$this->user->lang('MSG_ID'),
			$this->user->lang('FROM'),
			$this->user->lang('TO'),
			$this->user->lang('PMREAD_EXPORT_COL_BCC'),
			$this->user->lang('DATETIME'),
